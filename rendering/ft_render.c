...
/*   Updated: 2025/08/11 04:52:36 by souaguen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    t_vec3 world_up = ft_vec3(0, 1, 0);
    t_vec3 right;
    t_vec3 up;

    forward = ft_normalize(forward);

    // Si forward est parallèle à world_up, on choisit un autre up
    ...
    matrix[0] = right;    // axe X caméra → monde
    matrix[1] = up;       // axe Y caméra → monde
    matrix[2] = forward;  // axe Z caméra → monde
}
...
t_vec3 transform_ray(t_vec3 vector, t_vec3 matrix[3])
{
	...
	world_ray.x = vector.x * matrix[0].x + vector.y * matrix[1].x + vector.z * matrix[2].x;
	world_ray.y = vector.x * matrix[0].y + vector.y * matrix[1].y + vector.z * matrix[2].y;
	world_ray.z = vector.x * matrix[0].z + vector.y * matrix[1].z + vector.z * matrix[2].z;
